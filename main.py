...
async def create_shipment_request(
    ...
    try:
        ...
        if send_opt == "category":
            ...
            target_carriers = [c for c in all_customer_carriers if c.get("category_id") in cat_ids]
        elif send_opt == "selected" or not payload.send_to_all:
            selected_ids = set(payload.carrier_ids or [])
            target_carriers = [c for c in all_customer_carriers if c.get("id") in selected_ids]
        else:
            target_carriers = all_customer_carriers

        if not target_carriers:
            raise HTTPException(status_code=400, detail="Seçilmiş kriteriyalara (kateqoriya və ya siyahı) uyğun daşıyıcı tapılmadı.")
...
async def check_bounces_endpoint():
    if not ENABLE_BOUNCE_CHECK:
        return {
            "status": "disabled",
            "message": "Bounce yoxlaması hazırda deaktivdir (Amazon SES-ə keçid səbəbindən)."
        }
    try:
        result = await asyncio.to_thread(check_bounced_emails)
